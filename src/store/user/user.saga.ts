...
export function* getSnapshotFromUserAuth(
  ...
  try {
    ...
    // Do not mask Firestore failures as silent success — surface the error.
...
export function* signInWithEmail({
  ...
  try {
    ...
    const userCredential = yield call(
      firebaseUtils.signInAuthUserWithEmailAndPassword,
      email,
      password,
    );
    if (userCredential) {
...
export function* signUp({
  ...
  try {
    const firebaseUtils = yield call(loadFirebaseUtils);
    const userCredential = yield call(
      firebaseUtils.createAuthUserWithEmailAndPassword,
      email,
      password,
    );
    if (userCredential) {
